$(document).ready(function () {
    ...
    var table = $('#datatable').DataTable({
        ...
        "serverSide": true,
        ajax: {
            url : domain + 'location/getLocationData',
            type:'post',
            data : function(d) {
                d.parent_id = filterForm.find('input#token-input-location').val();
                d.group_id = filterForm.find('input#token-input-location-group').val();
                /*d.bussiness_id = filterForm.find('select[name="bussiness_id"]').select2().val();
                d.start_date = filterForm.find('input#start-date').val();
                d.end_date = filterForm.find('input#end-date').val();
                d.search.value = filterForm.find('input[name="name"]').val();*/
            },
            dataSrc : 'data'
        },
        columns: [
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function (data, type, full, meta){
                    var txt = '<a class="" href="'+baseUrl+controller+'/index/?parent_id='+full.id+'" title="Danh sách trực thuộc">';
                    txt += data;
                    txt += '</a>';
                    return txt;
                }
...
function bunk_addnew() {
    ...
    location.href=baseUrl+control+"/edit/?"+currentUrl[1];
}
