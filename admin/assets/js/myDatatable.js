$(document).ready(function () {
    ...
    if(controller=='game') {
        var table = $('#datatable').DataTable({
            ...
            buttons: [
                ...
                {text: '<i class="fa fa-usd"></i> Cập nhật giá', action: bunk_update_price, className:"btn btn-sm btn-info"},
                {text: '<i class="fa fa-lock"></i> Khóa', action: bunk_lock, className:"btn btn-sm btn-warning"},
    ...
    if(controller=='product_change_price') {
        var table = $('#datatable').DataTable({
            dom: "Bfrtip",
            pageLength: 1000,
            "oLanguage": {
...
function bunk_addnew() {
    ...
    location.href=baseUrl+"admin/"+control+"/edit";
}
...
function exec_delete(dt) {
    ...
    $.ajax({
        url: baseUrl+'admin/'+controller+'/delete',
        type:'post',
        dataType:'json',
        data:{ids:ids}
    }).done(function(resp){
...
function bunk_savePosition() {
    var confirm_msg = 'Bạn muốn sắp xếp lại vị trí hiển thị?';
    var buttons = { 
        'Ok' : function(){ exec_savePostion(); },
        'Cancel': function(){ $(this).dialog('close'); }
    };
    confirm_dialog(confirm_msg,buttons);
}

function exec_savePostion() {
    var data = [];
    $('.auto-order-changed').each(function(index){
        data[$(this).data('id')] = $(this).val();
    });
    $.ajax({
        url : baseUrl+'cms/playlist/savePosition',
        type : 'post',
        dataType:'json',
        data:{display_position:data}
    }).done(function(res){
        if(res.code==0) {
            location.reload();
        } else {
...
function bunk_pick_hot(e,dt,node,config) {
    var confirm_msg = 'Bạn muốn đánh dấu clip HOT?';
    var buttons = { 
        'Ok' : function(){ exec_pick_hot(dt); },
        'Cancel': function(){ $(this).dialog('close'); }
    };
    confirm_dialog(confirm_msg,buttons);
}

function bunk_unpick_hot(e,dt,node,config) {
    var confirm_msg = 'Bạn muốn đánh hủy clip HOT?';
    var buttons = { 
        'Ok' : function(){ exec_unpick_hot(dt); },
        'Cancel': function(){ $(this).dialog('close'); }
    };
    confirm_dialog(confirm_msg,buttons);
}

function exec_pick_hot(dt) {
    var controller = $('#datatable').data('controller');
    var sRows = dt.rows('.selected');
    var dts = sRows.data();
    var ids = [];
    ...
    $.ajax({
        url: baseUrl+'admin/'+controller+'/pick_hot',
        type:'post',
        dataType:'json',
        data:{ids:ids}
    }).done(function(resp){
...
function exec_unpick_hot(dt) {
    ...
    $.ajax({
        url: baseUrl+'admin/'+controller+'/remove_hot',
        type:'post',
        dataType:'json',
        data:{ids:ids}
    }).done(function(resp){
...
function bunk_update_price() {
    var cat_id = $('select#filter-type').val();
    if(parseInt(cat_id)===0) {
        showMessage('Bạn phải chọn chuyên mục để thực hiện thao tác này');
        return false;
    }
    location.href = adminUrl + '/' + $('#datatable').data('controller') + '/change_price/?cat_id='+cat_id;
}