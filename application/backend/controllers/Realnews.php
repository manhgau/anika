<?php
    ...
    class Realnews extends MY_Controller {        
        public function __construct() 
        {
            ...
            //Breadcrumbs
            $this->data['breadcrumbs']['Bài viết'] = base_url('realnews');
        }

        public function index() 
        {
            //load view template
            $this->data['meta_title'] = 'Bài viết';
            $this->data['sub_view'] = 'admin/realnews/index';
            $this->data['sub_js'] = 'admin/realnews/index-js';
        ...
        {
            $q = $_GET['q'];
            $list_product = $this->realnews_model->get_by('name LIKE "%'.$q.'%"');
        ...
        {
            $this->data['news'] = ($id) ? $this->realnews_model->get($id) : $this->realnews_model->getNew();

            //validate form
            $rules = $this->realnews_model->rules;
            $this->form_validation->set_rules($rules);
            if($this->form_validation->run() == TRUE) {
                $data = $this->realnews_model->array_from_post(array('title','intro', 'address', 'province_id', 'district_id', 'owner_name', 'owner_phone','content','service_time', 'type','status','is_public','price', 'rent_price_hour', 'rent_price_day', 'rent_price_month','sale_bonus','point', 'code', 'rent_time', 'bedroom_number', 'floor_number', 'acreage', 'fb_page_url', 'member_id', 'rent_enddate'));

                $data['rent_enddate'] = ($data['rent_enddate']) ? date('Y-m-d', strtotime( str_replace('/', '-', $data['rent_enddate']) )) : null;

                $data['price'] = $data['price'] * TYDONG;
                $data['rent_price_hour'] = $data['rent_price_hour'] * TRIEUDONG;
                $data['rent_price_day'] = $data['rent_price_day'] * TRIEUDONG;
                $data['rent_price_month'] = $data['rent_price_month'] * TRIEUDONG;
                $data['sale_bonus'] = $data['sale_bonus'] * TRIEUDONG;
                
                $data['service_type'] = [];
                $data['rent_time'] = [];
                if ($data['price']) {
                    $data['service_type'][] = $this->realnews_model::SEVICE_MUABAN;
                }
                
                if ($data['rent_price_hour'] || $data['rent_price_day'] || $data['rent_price_month']) {
                    $data['service_type'][] = $this->realnews_model::SEVICE_CHOTHUE;
                    if ($data['rent_price_hour']) 
                        $data['rent_time'][] = 'gio';
                    if ($data['rent_price_day']) 
                        $data['rent_time'][] = 'ngay';
                    if ($data['rent_price_month']) 
                        $data['rent_time'][] = 'thang';
                }
                $data['service_type'] = json_encode($data['service_type']);
                $data['rent_time'] = json_encode($data['rent_time']);

                $data['point'] = intval(str_replace(',', '', $data['point']));
                ...
                if($id = $this->realnews_model->save($data,$id)) {
                ...
                redirect(base_url('realnews'));
            }

            //Load View
            $this->data['meta_title'] = ($id) ? 'Sửa bài viết' : 'Viết bài mới';            
            $this->data['sub_view'] = 'admin/realnews/edit';
            $this->data['sub_js'] = 'admin/realnews/edit-js';
            $this->load->view('admin/_layout_main',$this->data);
        }

        public function delete($id = NULL) 
        {
            $news = $this->realnews_model->get($id,true);
            if ( ! $this->has_permission('delete',$news->category)) $this->not_permission();     
            $post_id = $this->input->post('ids');
            if($id) {
                $post_id[] = $id;
            }
            if($this->realnews_model->updateStatus($post_id,3)) {
        ...
        public function _unique_slug($str) 
        {
            ...
            $article = $this->realnews_model->get();
            if(count($article)) {
        ...
        {
            $result = $this->realnews_model->dataGrid();
        ...
        private function __removeNews()
        {
            ...
            $this->realnews_model->removeNews($id);
        ...
        {
            $id = intval($this->input->post('id'));
            $news = $this->realnews_model->get($id);
            $data = [
                'is_public' => ($news->is_public) ? 0 : 1
            ];
            $this->realnews_model->save($data, $id);
            $this->jsonResponse(200, 'success');
        }
