...
<script>
    ...
    $(document).on('click','#quick-search-result li', function(){
        if($('#sortable li').length >= 5) {
            alert('Bạn chỉ được chọn tối đa 05 bài viết');
            return false;
        }
        var id = $(this).data('id');
        var name = $(this).html();
        var selected = '<li class="ui-state-default"><input type="hidden" name="home_feature[]" value="'+id+'">'+name+'</li>';
