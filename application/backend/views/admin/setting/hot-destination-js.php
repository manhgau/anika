...
<script>
    ...
    $(document).on('click','#quick-search-btn', function(){
        ...
        $.get(domain + 'apis/token_search_location/?q='+keyword+'&type=transfer', function(resp){
    ...
    $('#quick-search-input').on('change', function() {
        $('#quick-search-btn').trigger('click');
    });
    
    //Select news
    $(document).on('click','#quick-search-result li', function(){
        if($('#sortable li').length >= 6) {
            alert('Bạn chỉ được chọn tối đa 06 bài viết');
            return false;
        }
        var id = $(this).data('id');
        var name = $(this).html();
        var selected = '<li class="ui-state-default"><input type="hidden" name="hot_destination[]" value="'+id+'">'+name+'</li>';
