...
<script>
    ...
    $(document).on('click','#quick-search-btn', function(){
        ...
        $.get(domain + 'apis/token_search_video/?q='+keyword, function(resp){
    ...
    $(document).on('click','#quick-search-result li', function(){
        ...
        {
            alert('Sorry! Chỉ được chọn tối đa 06 bài viết');
            return false;
        }
        var id = $(this).data('id');
        var name = $(this).html();
        var selected = '<li class="ui-state-default"><input type="hidden" name="static_videos[]" value="'+id+'">'+name+'</li>';
